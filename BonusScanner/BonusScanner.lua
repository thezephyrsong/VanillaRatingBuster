...
BONUSSCANNER_VERSION = "v1.1";


BONUSSCANNER_PATTERN_SETNAME = "^(.*) %(%d/%d%)$";
BONUSSCANNER_PATTERN_ARMOR_PREFIX = "^(%d+)%%?(.*)$";
...
function BonusScanner:CheckOther(line)
	...
	-- -- Scan mana oils
	-- for i,p in BONUSSCANNER_PATTERNS_OILS do
	-- 	matched = string.find(line, "^" .. p.pattern);
	-- 	if(matched) then
	-- 		BonusScanner:Debug("Matched : " .. p.effect .. " : " .. p.value .. "");
